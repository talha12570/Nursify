...
import { wp, hp, fp, spacing, componentSizes, shadows, device } from '../utils/responsive';
...
export default function CaregiverDashboard({ onViewJobDetails, onViewActiveBooking, onBack, onLogout, onSetProfile, onViewProfile } = {}) {
  ...
  // GPS location – sent with each heartbeat so patients can find this caregiver
  const { location: caregiverLocation } = useLocation({
    autoRequest: true,
    watch: true,
    watchTimeIntervalMs: 1000,
    watchDistanceIntervalM: 1,
  });
  // Keep a ref so sendHeartbeat can access the latest value without re-creating the interval
  const caregiverLocationRef = useRef(null);
  useEffect(() => {
    caregiverLocationRef.current = caregiverLocation;
  }, [caregiverLocation]);
  ...
  useEffect(() => {
    ...
    // Send heartbeat every second to keep caregiver location highly fresh
  ...
  const sendHeartbeat = async () => {
    try {
      ...
      const loc = caregiverLocationRef.current;
      const body = loc
        ? { latitude: loc.lat, longitude: loc.lng, accuracy: loc.accuracy }
        : {};

      await axios.post(`${API_URL}/caregiver/heartbeat`, body, {
        headers: {
  ...
  const fetchActiveBookings = async () => {
    try {
      ...
      // Fetch accepted, confirmed, and in-progress bookings
      const statuses = ['approved', 'confirmed', 'on_the_way', 'arrived', 'service_started', 'service_completed'];
  ...
  const handleAcceptBooking = async (bookingId) => {
    try {
      ...
      if (response.data.success) {
        Alert.alert('Success', 'Booking request accepted. Service tracking is now active.');
        if (response.data.booking) {
          onViewActiveBooking?.(response.data.booking);
        }
  ...
  const getStatusColor = (status) => {
    const colors = {
      'approved': { backgroundColor: '#cffafe' },
            ...
            <View style={styles.headerTop}>
              <View style={styles.headerLeft}>
                ...
                <View style={styles.profileSection}>
                  ...
                  <Text style={styles.profileNameText} numberOfLines={2}>{userName}</Text>
                </View>
              </View>
              <View style={styles.headerActions}>
                ...
                <TouchableOpacity style={styles.iconButton} activeOpacity={0.7} onPress={handleLogout}>
                  <Svg width={20} height={20} viewBox="0 0 24 24" fill="none" stroke="#ffffff" strokeWidth={2}>
                ...
                {confirmedBookings.map((booking) => (
                  ...
                  >
                    ...
                    <View style={styles.bookingInfo}>
                      <Text style={styles.bookingPatient} numberOfLines={1}>{booking.patient?.fullName || 'Patient'}</Text>
                      <Text style={styles.bookingService} numberOfLines={1}>{booking.serviceType}</Text>
                    ...
                    {allBookings.map((booking) => (
                      <TouchableOpacity 
                        ...
                        onPress={() => {
                          ...
                          } else if (['approved', 'confirmed', 'on_the_way', 'arrived', 'service_started', 'service_completed'].includes(booking.status)) {
...
const styles = StyleSheet.create({
  ...
  profileMenuContainer: {
    backgroundColor: '#ffffff',
    borderRadius: wp(5),
    width: Math.min(wp(85), device.width - spacing.xl * 2),
    ...shadows.large,
  },
  profileMenuHeader: {
    flexDirection: 'row',
